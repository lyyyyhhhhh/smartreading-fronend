<template>
  <view>
    ...
    <uni-nav-bar left-icon="left" @clickLeft="backLast" :border="false" height="60px" title="我的收藏"></uni-nav-bar>
    <view v-if="mycollectionarticleinfolist.length === 0">
      <u-empty mode="data" icon="http://cdn.uviewui.com/uview/empty/data.png"
               style="position: fixed; top: 400rpx; left: 33%;" text="您还没有收藏任何文章哦!" textSize="12">
      </u-empty>
    </view>
    <view v-else>
      <view v-for="(item, index) in mycollectionarticleinfolist" :key="index"
  ...
  onLoad() {
    ...
    uni.request({
      ...
      success: res => {
        ...
        // 拼接出查询条件
        var selectids = [];
        this.collectedtimes.length = 0;
        for (let i = 0; i < res.data.length; i++) {
          selectids.push(res.data[i].articleid);

          this.collectedtimes.push(res.data[i].createTime);
        }
        ...
        uni.request({
          ...
          success: res => {
            ...
            this.mycollectionarticleinfolist.length = 0;
            // 整理结果
            for (let i = 0; i < res.data.length; i++) {

              let collectedtime = new Date(Date.parse(this.collectedtimes[i]));
              ...
              this.mycollectionarticleinfolist.push(articleinfo);
